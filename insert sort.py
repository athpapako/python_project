...
# ανοίγει το αρχείο config.txt σε ένα νέο παράθυρο για επεξεργασία
# με αυτό τον τρόπο μπορούμα να αλλάξουμε τις τιμές των παραμέτρων σχεδίασης
...
# εκτυπώνει τα στοιχεια της λίστας στο κάτω μέρος της οθόνης
def writeList(v):
...
def insert(v,k):
    x = v[k]
    draw_bars(v,n,k,k,2) # σχεδιάζει με διαφορετικό χρώμα την μπάρα του στοιχειου k 
    time.sleep(DELAY)
    for i in range(k-1,-1,-1):
        if v[i] > x:        # μετακίνηση των στοιχείων που ειναι μεγαλύτερα  
            v[i+1] = v[i]   # μία θέση δεξιά
            draw_bars(v,n,i+1,k,1) # σχεδιάζει το δαβδοδιάγραμμα
        else:
            v[i+1] = x             # μεταφάρει το στοιχείο στη νέα θέση 
            draw_bars(v,n,i+1,k,2) # σχεδιάζει με διαφορετικό χρώμα την μπάρα του στοιχειου 
            time.sleep(DELAY)
            break
...
def sort_with_graph(v):
        global screen
        screen=setupscreen(SCREENX,SCREENY)
        insertion_sort1(v)
        draw_bars(v,len(v))
        time.sleep(3)
        turtle.bye()
